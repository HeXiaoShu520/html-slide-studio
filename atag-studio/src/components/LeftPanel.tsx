  ...
  return (
    <div className="w-56 bg-[var(--atag-bg-panel)] border-r border-[var(--atag-border)] flex flex-col shrink-0 overflow-hidden">
      <div className="flex border-b border-[var(--atag-border)]">
        <TabBtn active={leftPanelTab === 'blocks'} icon={<LayoutGrid size={14} />} label="组件" onClick={() => setLeftPanelTab('blocks')} />
        <TabBtn active={leftPanelTab === 'layers'} icon={<Layers size={14} />} label="图层" onClick={() => setLeftPanelTab('layers')} />
      </div>
